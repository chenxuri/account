...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            url: baseURL+'/spider/accountInfoWhite/list',     //请求后台的URL（*）
            method: 'get',           //请求方式（*）
...
function add() {
    ...
    if(addParsley.isValid()){
        var saveData = { organization:{
            id:$("#form-add").find("select[name='orgId'] option:selected").val(),
            name:$("#form-add").find("select[name='orgId'] option:selected").text()
        },
            accountId:$("#form-add").find("input[name='accountId']").val(),
            accountName:$("#form-add").find("input[name='accountName']").val(),
            nickname:$("#form-add").find("input[name='nickname']").val(),
            type:$("#form-add").find("select[name='type'] option:selected").val(),
            typeName:$("#form-add").find("select[name='type'] option:selected").text(),
        ...
        $.ajax({
            type:"POST",
            url:baseURL+"/spider/accountInfoWhite/add",
            dataType:"json",
...
function remove() {
    ...
    $.ajax({
        type:'post',
        traditional :true,
        url:baseURL+'/spider/accountInfoWhite/delete',
        data:{ids:idAr},
        success:function(data){
            if(data == '1'){
                alert("删除成功");
